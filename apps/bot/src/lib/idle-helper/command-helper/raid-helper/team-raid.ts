...
export const _teamRaidHelper = async ({
  client,
  channelId,
  users,
  collected
...
const generateMessageOptions = ({
  ...
  for (const member of raidInfo.members) {
    ...
    if (!workersInfo.length) workersInfo.push('No workers registered');
  ...
  embed.setDescription(
    `Timer: ${timestampHelper.relative({
      time: new Date(startTime.getTime() + TEAM_RAID_DURATION)
    })}`
  );
